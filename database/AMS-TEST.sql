...
/* ---- 4. Seeded Supreme Root login -------------------------------------------
   Login name looks like a low-level operator account but holds Supreme Root
   rights. The API inserts the same account automatically if it is missing, so
   this block is a no-op when the API already created it. */
...
BEGIN
    ...
    VALUES (N'operator.sys', N'<hash-set-by-api>', N'<salt-set-by-api>', N'Supreme Root', NULL, N'operator.sys@ams.local', N'Portal account (looks like a low-level operator login, but holds Supreme Root rights).', 1);
END
GO
